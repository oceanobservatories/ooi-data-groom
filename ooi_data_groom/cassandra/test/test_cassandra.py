...
from ..data import insert_dataframe, fetch_bin
...
class CassandraDataUnitTest(unittest.TestCase):
    ...
    def test_02_fetch_bin(self):
        df = fetch_bin('test', 'test', 'test', 'test', 'botpt_nano_sample', 3682368000, ['time', 'bottom_pressure'])
        self.assertEqual(df.time.size, 216000)
